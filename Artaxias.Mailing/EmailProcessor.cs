    ...
    public class EmailProcessor : IEmailProcessor
    {
        ...
        public async Task SendAsync(EmailConfigurationParameters emailMessage)
        {
            try
            {
                ...
                message.To.AddRange(emailMessage.DestinationAddresses.Select(x => MailboxAddress.Parse(x)));
                message.From.AddRange(emailMessage.SourceAddresses.Select(x => MailboxAddress.Parse(x)));
                message.Cc.AddRange(emailMessage.CcAddresses.Select(x => MailboxAddress.Parse(x)));
                message.Bcc.AddRange(emailMessage.BccAddresses.Select(x => MailboxAddress.Parse(x)));
