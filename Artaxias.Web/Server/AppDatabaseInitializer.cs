...
namespace Artaxias.Web.Server
{
    public class AppDatabaseInitializer
    {
        ...
        {
            AnswerOptionType answerOptionType = await _answerOptionTypeRepository.Get(x => x.Description == type).FirstOrDefaultAsync();
            if (answerOptionType != null)
            {
                return;
            }
        ...
        private async Task CheckDomainStateComplianceAsync(EDomainState state)
        {
            ...
            if (domainState != null)
            {
                if (domainState.Description.Equals(stateName))
                {
                    return;
                }
        ...
        {
            User user = await _userManager.FindByNameAsync("user");
            if (user == null)
            {
            ...
            await AssertUserRolePermissions();
        }
        ...
        {
            Role role = await _roleManager.FindByNameAsync(nameof(EAuthorizationRoles.Administrator));
        ...
        private async Task AssertUserRolePermissions()
        {
            Role role = await _roleManager.FindByNameAsync(nameof(EAuthorizationRoles.User));
        ...
        private static IEnumerable<string> GetTypeDescriptions(Type type)
        {
            ...
            foreach (string name in names)
            {
                ...
                object[] fields = field?.GetCustomAttributes(typeof(DescriptionAttribute), true);
                if (fields == null)
                {
                    continue;
                }
        ...
        private async Task AssertAbsenceTypesExistence()
        {
            ...
            foreach (string description in employeeAbsenceTypeDescriptions)
            {
        ...
        {
            AbsenceType absenceType = _absenceTypeRepository.Get(x => x.Description == description).FirstOrDefault();
            if (absenceType != null)
            {
                return;
            }
